...
- Reuse existing DRY helpers — don't re-duplicate their pattern: `_connect_device()`,
  `Gamepad._read_indices()`, `_load_optional[T]()` in `car/settings.py`, and
  `DriveToggle`/`direction_to_forward()`.
- Use `ui.Dashboard` for per-tick telemetry, never plain `console.print()`. Pass the matching
  `_FREE_DRIVE_CONTROLS`/`_TOGGLE_CONTROLS` tuple to every `dashboard.update(..., controls=)`
  call so the live display always reflects the active mode.
...
| Tool | Command | Notes |
| --- | --- | --- |
| pytest | `python -m pytest` | Pure-logic modules only (`drive.py`, `track.py`, `boundary.py`) |
| ruff | `ruff check car/ tests/` / `ruff format car/ tests/` | Lint and format, line length 130 |
| pymarkdown | `pymarkdown --config pyproject.toml scan README.md AGENTS.md` | Markdown lint |
